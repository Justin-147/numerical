# -*- coding: utf-8 -*-
"""
R 值计算与成图：读取已含时间列与特征列的数据文件(通常该数据来自异常提取方法的处理输出结果)，以及地震目录、规则文件、干扰信息，
在阈值×预测期网格上计算 R 值，输出结果文件与图件。
图件布局：左上=空间分布，右上=R-TT 二维，左下=残差+阈值+预警+震例。
"""
...
# ===========================
# 四、成图（左上=地图，右上=R-TT，左下=残差+预警+震例）
# ===========================
...
def plot_result_figure(
    ...
    fig.suptitle("R-value result", fontsize=14)
...
def run_rvalue(
    ...
    print("数据读取完成（时间列 + 数值列），直接进行 R 值计算。")
    ...
    if interference_file and interf_start.size > 0:
        print(f"成功读取 {len(interf_start)} 段干扰时段，将在异常提取和 R 值计算中剔除。")
    else:
    ...
    with open(out_txt, "w", encoding="utf-8") as f:
        ...
        if threshold_mode == 1:
            ...
            f.write(f"{len(ab_st_best):6d} {success:16d} {miss:16d} {Rmax:8.4f} {R0:8.4f} {best_alm_day:4d} {best_threshold:18.2f} {mag_min:8.1f} {mag_max:8.1f}\n")
        else:
...
if __name__ == "__main__":
    ...
    if test_example == 1:
        DATA_FILE = Path(r"d:\numerical\R-value\32016_3_2221_cycle_related_anomaly.txt") # 待评估数据文件
        POLYGON_FILE = Path(r"d:\numerical\R-value\32016_3_2221_EQ_SelectRule.txt") # 地震筛选规则文件
        EQ_CATALOG_FILE = Path(r"d:\numerical\R-value\china3msN.eqt") # 地震目录文件
        INTERFERENCE_FILE = Path(r"d:\numerical\R-value\32016_3_2221_Interference_Period.txt") # 干扰信息文件
        ...
        DATA_FILE = Path(r"d:\numerical\R-value\14001_1_2231_trend_related_anomaly.txt") # 待评估数据文件
        POLYGON_FILE = Path(r"d:\numerical\R-value\14001_1_2231_EQ_SelectRule.txt") # 地震筛选规则文件
        EQ_CATALOG_FILE = Path(r"d:\numerical\R-value\china3ms.eqt") # 地震目录文件
        INTERFERENCE_FILE = Path(r"d:\numerical\R-value\14001_1_2231_Interference_Period.txt") # 干扰信息文件
